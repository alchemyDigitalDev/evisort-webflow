...
const manualAccordionItemTime = 7500
let currentItemID = 1

function closeManualAccordionItem(closeItem) {
  $(closeItem).addClass('accordion-item---closed')
...
function showManualAccordionItem(accordion_items, openItem) {
  // Hide other accordion items
  $(accordion_items)
    .not(openItem)
    .each(function (index, accordion_item) {
      closeManualAccordionItem(accordion_item)
    })
    .promise()
    .done(function () {
      // Show current one
      ...
      let current_quote = $(openItem)
        .closest('.module---accordion-manual')
        .find(
          '.accordion-tile-quote-wrap---desktop.accordion-tile-quote-wrap-' +
            current_id
        )
...
function startAccordionAnimation(accordion, screenWidth, itemsTimeout) {
  ...
  if (accordion_items) {
    $(accordion_items).each(function (index, accordion_item) {
      closeManualAccordionItem(accordion_item)
      let currentTimeoutTime = manualAccordionItemTime * index
      if (screenWidth >= 992) {
        itemsTimeout.push(
          setTimeout(function () {
            showManualAccordionItem(accordion_items, accordion_item)
          }, currentTimeoutTime)
        )
      }
      $(accordion_item).click(function () {
        ...
        showManualAccordionItem(accordion_items, accordion_item, true)
      })
    })

    if (screenWidth < 992) {
      showManualAccordionItem(accordion_items, accordion_items[0], true)
    }
  }
}
...
function manualAccordionsInit() {
  const accordions = $('.module---accordion-manual .grid')
  let screenWidth = window.innerWidth
  let itemsTimeout = []
